...
const Dashboard = () => {
  ...
  const disabled =
    currentPage === Math.ceil(list?.length / dataPerPage) ? true : false;
  ...
  const getDataPartner = async () => {
    await axios
      .get(
        `https://irisminty.my.id/partners/${localStorage.getItem(
          "partner_id"
        )}`,
        {
          headers: {
  ...
  const getListServices = () => {
    apiWithAuth(
      `partners/${parseInt(partnerId)}/services`,
      `GET`,
      null,
      "application/json",
      token
          ...
          <div className="flex flex-col justify between">
            <div>
              {status == "Not Verified" || status == "Revision" ? (
                <div>
                  <h1 className="text-xl font-bold text-bozz-one mb-5 border border-bozz-one p-5 rounded-xl">
                    Account Not Verify Yet
                  </h1>
                  <h1 className="text-xl font-bold text-bozz-one">
                    Verify Progress
                  </h1>
                  <ul className="steps steps-vertical">
                    {logs?.map((log, index) => {
                      return (
                        <li
                          className="step step-primary text-bozz-one capitalize"
                          key={index}
                        >
                          <span>{log}</span>
                        </li>
                    ...
                    {status == "Revision" ? (
                      <Link
                        to="/partner/revisi-registrasi"
                        className="text-bozz-two font-bold underline"
                      >
                        {" "}
                        Link Revisi
                      </Link>
              ...
              {status == "Verified" && !listServices ? (
                <div className="flex flex-col mt-3">
                  <button
                    className="bg-bozz-two text-bozz-six h-8 py-0 px-8 rounded-lg mb-5 self-end"
                  ...
                  <button
                    className="bg-bozz-two text-bozz-six h-8 py-0 px-8 rounded-lg mb-5 self-end"
                    onClick={() => navigate("/partner/add-additional")}
                  >
                    {" "}
                    Add New Additional
                  </button>
                  <h1 className="text-xl font-bold text-bozz-one mb-5 border border-bozz-one p-5 rounded-xl">
                    List Services Not Add Yet
                  </h1>
                </div>
              ) : null}
              {status == "Verified" && listServices ? (
                <div className="flex flex-col mt-3">
                  ...
                  <div className="flex">
                    <div
                      className={`w-40 text-sm h-8 text-center py-2 ${
                        active == "service"
                          ? `bg-bozz-two text-white font-semibold`
                          : `bg-bozz-six border border-bozz-two text-bozz-two font-semibold`
                      }`}
                      ...
                      <table className="w-full table-auto">
                        <thead className="border-b-2 border-bozz-one">
                          <tr className="text-center">
                            {serviceHead?.map((title, index) => {
                              return (
                                <td
                                  className="text-bozz-three font-semibold capitalize"
                                  key={index}
                                >
                                  {title}
                                </td>
                        ...
                        <tbody>
                          {current.map((data, index) => {
                            return (
                              <tr
                                className="text-bozz-three border-b border-bozz-three h-8 text-xs text-center py-10"
                                key={index}
                              >
                                <td>{index + 1}</td>
                                <td>
                                  <a onClick={() => serviceDetail(data.id)}>
                                    {data.service_name}
                                  </a>
                                </td>
                        ...
                        <tbody>
                          {listAdditional &&
                            listAdditional?.map((data, index) => {
                              return (
                                <tr
                                  className="text-bozz-three border-b border-bozz-three h-8 text-xs text-center"
                                  key={index}
                                >
                                  <td>{index + 1}</td>
                                  <td>{data.additional_name}</td>
                                  <td>
                                    {formatCurrency(data.additional_price)}
                                  </td>
                                  <td className="flex justify-evenly items-center py-3 text-lg">
                                    <FaEdit
                                      className="text-bozz-two"
                                      onClick={() => goEditAdd(data)}
                                    />
                                    <FaTrashAlt
                                      className="text-red-400"
                                      onClick={() => deleteAdditional(data.id)}
                                    />
                                  </td>
                                </tr>
                              );
                ...
                {status == "Verified" && pages
                  ? pages?.map((page, index) => {
                      return (
                        <button
                          key={index}
                          className="h-8 w-8 focus:bg-bozz-two focus:text-white border border-bozz-two bg-white text-bozz-two hover:text-white hover:bg-bozz-two btn-circle"
